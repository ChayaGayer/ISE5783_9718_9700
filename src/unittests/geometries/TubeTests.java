...
class TubeTests {
	...
	void testGetNormal() {
		...
		Ray v = new Ray( new Vector(1, 0, 0),p0);
		double rad = 2;
	...
	public void testFindIntersections() {
		Tube tube1 = new Tube(new Ray( new Vector(2, 2, 0),new Point(0, -1, 0)), 1);
		Tube tube = new Tube(new Ray( new Vector(1, 0, 0),new Point(0, 2, 0)), 1);
		// ============ Equivalence Partitions Tests ==============
		// TC01: the ray starts from inside, intersects with the tube (1 point)
		List<Point> result123 = tube.findIntersections(new Ray( new Vector(2, -1.5, 0.5),new Point(2, 1.5, 0.5)));
		...
		List<Point> result2 = tube.findIntersections(new Ray( new Vector(1, 2.5, 1),new Point(1, 0, 0)));
		...
		assertNull(tube1.findIntersections(new Ray( new Vector(2, 0, 1),new Point(-1, 1, 1))), "Ray out of tube");

		// TC04: the ray starts from outside, doesn't intersect with the tube (0 points)
		assertNull(tube.findIntersections(new Ray( new Vector(1, 2.5, 1),new Point(2, 2.5, 1))), "Ray out of tube");

		// TC05: should've intersected but is outside of max distance boundries
		assertNull(tube.findGeoIntersections(new Ray(new Vector(1, 2.5, 1),new Point(1, 0, 0)), 0.5),
				"Ray out of tube");

		// =============== Boundary Values Tests ==================
		// *************parallel******************
		// TC11: the ray is parallel to the tube from the outside
		assertNull(tube1.findIntersections(new Ray( new Vector(1, 1, 0),new Point(1, 0, 2))), "Ray's parallel to tube");

		// TC12: the ray is on top of the tube's surface and is parallel to the tube
		// axis ray
		assertNull(tube.findIntersections(new Ray( new Vector(2, 0, 0),new Point(1, 2, 1))),
				"Ray's parallel to tube and on top of its surface");

		// TC13: the ray is inside and is parallel to the tube
		// axis ray
		assertNull(tube.findIntersections(new Ray( new Vector(2, 0, 0),new Point(1, 2, 0.5))),
				"Ray's parallel to tube and on top of its surface");

		// **Group: merges with tube axis ray**//
		// TC14: the ray merges after axis ray point
		assertNull(tube.findIntersections(new Ray( new Vector(1, 0, 0),new Point(1, 2, 0))),
				"Ray's parallel to tube and on top of its surface");
		...
		assertNull(tube.findIntersections(new Ray(new Vector(1, 0, 0),new Point(0, 2, 0))),
				"Ray's parallel to tube and on top of its surface");

		/////// parallel going in opposite directions
		// TC17: the ray is parallel to the tube from the outside
		assertNull(tube1.findIntersections(new Ray( new Vector(-1, -1, 0),new Point(1, 0, 2))),
		...
		assertNull(tube.findIntersections(new Ray(new Vector(0, 0, 1),new Point(0, 2, 2))),
				"Ray's perpendicular to tube");
		// TC22: the ray is orthogonal to the tube (0 points)
		assertNull(tube.findIntersections(new Ray( new Vector(0, 0, 1),new Point(0, 2, 1))),
				"Ray's orthogonal to tube");

		// TC23: the ray is perpendicular to the tube from the inside (1 points)
		Point p4 = new Point(0, 2, 1);
		List<Point> result3 = tube.findIntersections(new Ray( new Vector(0, 0, 1),new Point(0, 2, 0.5)));
		...
		List<Point> result4 = tube.findIntersections(new Ray(new Vector(0, 0, 1),new Point(0, 2, 0)));
		...
		// point (1 points)
		List<Point> result5 = tube.findIntersections(new Ray( new Vector(0, 0, 1),new Point(0, 2, -0.5)));
		...
		List<Point> result55 = tube.findIntersections(new Ray( new Vector(0, 0, 1),new Point(0, 2, -1)));
		...
		List<Point> result6 = tube.findIntersections(new Ray(new Vector(0, 0, 1),new Point(0, 2, -2)));
		...
		assertNull(tube.findIntersections(new Ray( new Vector(1.5, 4, 0),new Point(1.5, 0, 1))),
				"Ray's tangent to tube, before tangent point");

		// TC32: at tangent point (0 points)
		assertNull(tube.findIntersections(new Ray(new Vector(1.5, 4, 0),new Point(2.25, 2, 1))),
				"Ray's tangent to tube, at tangent point");

		// TC33: after tangent point (0 points)
		assertNull(tube.findIntersections(new Ray( new Vector(1.5, 4, 0),new Point(3, 4, 1))),
		...
		List<Point> result7 = tube.findIntersections(new Ray( new Vector(1, 3, -3),new Point(1, 1, 1)));
		...
		List<Point> result8 = tube.findIntersections(new Ray(new Vector(1, 3, -3),p6));
		...
		List<Point> result9 = tube.findIntersections(
				new Ray( new Vector(1, 3, -3),new Point(1.238240344551198, 1.714721033653591, 0.285278966346409)));
		...
		List<Point> result10 = tube
				.findIntersections(new Ray( new Vector(1, 3, -3),new Point(1.333333333333334, 2, 0)));
		...
		List<Point> result11 = tube.findIntersections(
				new Ray( new Vector(1, 3, -3),new Point(1.465399710013591, 2.396199130040771, -0.396199130040771)));
		...
		assertNull(tube.findIntersections(new Ray( new Vector(1, 3, -3),p7)), "Ray's point on tube surface");

		// TC47: outside the tube, facing the outside (0 points)
		assertNull(tube.findIntersections(new Ray( new Vector(1, 3, -3),new Point(2, 4, -2))),
		...
		List<Point> result12 = tube
				.findIntersections(new Ray( new Vector(1, 3, -3),new Point(-0.33333333333333, 1, 1)));
		...
		List<Point> result13 = tube.findIntersections(new Ray( new Vector(1, 3, -3),p9));
		...
		List<Point> result14 = tube.findIntersections(
				new Ray( new Vector(1, 3, -3),new Point(-0.095092988781802, 1.714721033653591, 0.285278966346409)));
		...
		List<Point> result15 = tube.findIntersections(new Ray( new Vector(1, 3, -3),new Point(0, 2, 0)));
		...
		List<Point> result16 = tube.findIntersections(
				new Ray( new Vector(1, 3, -3),new Point(0.132066376680591, 2.396199130040771, -0.396199130040771)));
		...
		assertNull(tube.findIntersections(new Ray( new Vector(1, 3, -3),p10)), "Ray's point on tube surface");

		// TC56: outside the tube, facing the outside (0 points)
		assertNull(tube.findIntersections(new Ray( new Vector(1, 3, -3),new Point(0.66666666667, 4, -2))),
				"Ray's point outside the tube");

		// *********surface general bva**********
		// TC61: the ray starts from tube surface, intersects with the tube (1 point)
		// using p2 and p3
		List<Point> result20 = tube.findIntersections(new Ray( new Vector(1, 2.5, 1),p2));
		...
		// point)
		assertNull(tube.findIntersections(new Ray( new Vector(1, 2.5, 1),p3)), "Ray's point outside the tube");
